...
function wRequest(urlStr, bodyObj, headersInput) {
    return new Promise((resolve) => {
        ...
        // Perbaikan: Menyertakan 'port: 443' secara tertulis dan tegas
        const tlsOptions = {
            host: urlObj.hostname,
            port: 443, 
        ...
        // Sintaks pemanggilan koneksi TLS yang aman dari error missing argument
...
export default async function handler(req, res) {
    ...
    if (!bookId) return res.status(400).json({ status: false, error: "Missing bookId" });
    ...
    try {
        ...
        const body = {
            boundaryIndex: 0, index: -1, currencyPlaySource: "discover_175_rec",
            needEndRecommend: 0, currencyPlaySourceName: "首页发现_Untukmu_推荐列表", preLoad: false,
            rid: "", pullCid: "", enterReaderChapterIndex: 0, loadDirection: 0,
            startUpKey: crypto.randomUUID(), bookId: String(bookId)
        };
        const headers = buildHeaders(deviceId, androidId);

        const result = await wRequest("https://sapi.dramaboxvideo.com/drama-box/chapterv2/batch/load", body, headers);
        
        if (result.success) {
            const listEp = result.data?.data?.chapterList || [];
            listEp.sort((a, b) => a.chapterIndex - b.chapterIndex);
            return res.status(200).json({ status: true, data: listEp });
        } else {
